-- Safe Dual Authentication Schema Migration
...
-- Create RLS policies for user_profiles (safe approach)
DO $$
BEGIN
...
-- Create trigger for user_profiles (safe approach)
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE user_id = user_uuid AND user_profiles.profile_type = new_profile_type
  ) THEN
    RETURN FALSE;
  END IF;
...
BEGIN
  ...
  IF EXISTS (
    SELECT 1 FROM user_profiles 
    WHERE user_id = user_uuid AND user_profiles.profile_type = create_user_profile.profile_type
  ) THEN
...
DO $$
BEGIN
  RAISE NOTICE 'Dual authentication schema migration completed successfully!';
END $$;
